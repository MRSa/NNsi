/*============================================================================*
 *
 *  $Id: sslv2md5.c,v 1.2 2004/02/01 14:35:11 mrsa Exp $
 *  FILE: 
 *     sslv2md5.c
 *
 *  Description: 
 *     SSL v2通信用MD5Hashを求めるプログラム
 *
 *===========================================================================*/
#define SSLV2MD5_C
#include "local.h"
...
/*                                                                         */
/*=========================================================================*/
void CalcMD5(Char *string, UInt32 stringLen, Char *buffer, UInt32 *bufLen, NNshSSLv2Ref *sslRef)
{
    UInt16 len;
    if (sslRef == NULL)
    {
    ...
    len = stringLen;
    MD5Update(&(sslRef->context), string, len);
    MD5Final (sslRef->digest, &(sslRef->context), sslRef->PADDING);

    // データをコピー
    MemSet (buffer, *bufLen, 0x00);
    *bufLen = SSL2_MD5_HASH_LENGTH;
    MD5_memcpy(buffer, sslRef->digest, SSL2_MD5_HASH_LENGTH);
...
/*---------------------------------------------------------------------------*
    ...
    a multiple of 4.
 *---------------------------------------------------------------------------*/
static void MD5sub_Encode(UInt8 *output, UInt32 *input, UInt16 len)
{
  UInt16  i, j;
...
/*---------------------------------------------------------------------------*
    ...
    a multiple of 4.
 *---------------------------------------------------------------------------*/
static void MD5sub_Decode(UInt32 *output, UInt8 *input, UInt16 len)
{
  UInt16 i, j;
 ...
 *---------------------------------------------------------------------------*/
static void MD5_memcpy(UInt8 *output, UInt8 *input, UInt16 len)
{
    UInt16 i;
