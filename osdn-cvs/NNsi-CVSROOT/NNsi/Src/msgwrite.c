/*============================================================================*
 *
 *  $Id: msgwrite.c,v 1.78 2012/01/10 16:27:41 mrsa Exp $
 *
 *  FILE: 
 *     msgwrite.c
 *
                              ...
                              Char *tempArea, UInt16 areaSize, Char *defSubmit)
{
    ...
    while (ptr < (buffer + bufSize))
    {
        ...
        if (StrNCompare(ptr, "MESSAGE", 7) == NULL)
        {
            // メッセージは無視する。
            continue;
        }
        NNsh_DebugMessage(ALTID_INFO, "hidden name :",  ptr, 0);
        *data++ = '&';
        if (*ptr == '"')
        {
            ptr++;
...
static void openWriteConfigulation(void)
{
    ...
    
    {
        ControlType *chkObj;
        UInt16 checkValue;
        chkObj = FrmGetObjectPtr(diagFrm, FrmGetObjectIndex(diagFrm, CHKID_CLEAR_WRITE_COOKIE));
        checkValue = CtlGetValue(chkObj);
        if ((checkValue != 0)&&(btnId == BTNID_DIALOG_OK))
        {
            // Cookieをクリアする！
            MemSet ((NNshGlobal->NNsiParam)->cookieStoreBuffer, NNSH_WRITECOOKIE_BUFFER, 0x00);
            NNsh_DebugMessage(ALTID_ERROR, "CLEAR COOKIE ", " ", checkValue);
        }
    }    
...
static Char *checkWriteReply(Char *buffer, UInt32 bufSize, UInt32 command, UInt16 bbsType, Err *ret)
{
    ...
    *ret = OpenFile_NNsh(FILE_RECVMSG,
    ...
    // Cookieを抽出して記録する。
    appendWriteCookie((NNshGlobal->NNsiParam)->cookieStoreBuffer, NNSH_WRITECOOKIE_BUFFER, buffer, readSize);
...
/*-------------------------------------------------------------------------*/
/*   Function : appendWriteCookie                                          */
/*                                                   Cookieを追加する      */
/*-------------------------------------------------------------------------*/
void appendWriteCookie(Char *buffer, UInt32 bufSize, Char *data, UInt32 dataSize)
{
    UInt32 bufLen;
    Char *ptr, *top, *key, *cookie;
    Char tempBuf[BUFSIZE];

    cookie = buffer + StrLen(buffer);
    top = data - 1;
    while ((ptr = StrStr((top + 1), "Set-Cookie: ")) != NULL)
    {
        ptr = ptr + StrLen("Set-Cookie: ");
        ...
        if (bufLen > BUFSIZE)
        {
            // そんなに長いキー名称を付けることはないと思うけど、念のため...
            bufLen = BUFSIZE - 1;
        }
        MemSet(tempBuf, sizeof(tempBuf), 0x00);
        StrNCopy(tempBuf, ptr, bufLen);

        NNsh_DebugMessage(ALTID_INFO, "FIND COOKIE : ", tempBuf, 0);

        // Cookieが登録済みか調べる
        if (StrStr(buffer, tempBuf) != NULL)
        {
            // Cookieが登録済みのため、読み飛ばす (本当は更新のほうがよいのだが...
            NNsh_DebugMessage(ALTID_INFO, "ALREADY KW : ", tempBuf, 0);
            top = ptr;
            continue;
        }

        // Cookie がひとつもつけられていない状態かどうかを確認する
        if (*buffer != 'C')
        {
            // バッファの先頭だったら、キーワードを付ける
            StrCopy(buffer, "Cookie: ");
            cookie = buffer + StrLen(buffer);
        }
        else
        {
            // cookie と cookie の区切り部分をつける
            *cookie = ';';
            cookie++;
            *cookie = ' ';
            cookie++;
        ...
        // Cookie部分をコピーする
        while ((cookie < (buffer + bufSize))&&(ptr <= (data + dataSize))&&(*ptr != ';')&&(*ptr != ' '))
        {
            *cookie = *ptr;
            cookie++;
            ptr++;
...
static Err threadWrite_Message(void)
{
    ...
    Char                tmpBuf[BUFSIZE], *param, timeBuff[20];
      ...
      default:
        ...
        if ((NNshGlobal->NNsiParam)->writeSequence2 != 0)
        {
            ...
            ret = NNshHttp_comm(HTTP_SENDTYPE_GET_NOTMONA, param,
                                (NNshGlobal->NNsiParam)->cookieStoreBuffer, NULL, HTTP_RANGE_OMIT, HTTP_RANGE_OMIT,
                                NULL);
            if (ret != errNone)
            {
                // (タイムアウト以外の)通信エラー発生
        ...
        else
        {
            ...
            setBBSNameAndTime(subjDB.bbsType, bbsData.boardNick,
                              subBuf, subBufSize);
            StrCat(subBuf, "&kibi=dango&submit=%8F%91%82%AB%8D%9E%82%DE");
        }
#ifdef USE_SSL
        // OysterのセッションIDを追記する
    ...
    NNsh_DebugMessage(ALTID_INFO, "◎", (NNshGlobal->NNsiParam)->cookieStoreBuffer, 0);
    switch (subjDB.bbsType)
    {
      case NNSH_BBSTYPE_MACHIBBS:
      ...
      default:
        if ((NNshGlobal->NNsiParam)->writeSequence2 != 0)
        {
            ret = NNshHttp_comm(HTTP_SENDTYPE_POST_SEQUENCE2, url,
                                (NNshGlobal->NNsiParam)->cookieStoreBuffer, subBuf, HTTP_RANGE_OMIT, HTTP_RANGE_OMIT,
                                NULL);
        }
        else
        {
    ...
    if (ptr != NULL)
    {
        MemSet(timeBuff, sizeof(timeBuff), 0x00);
        ptr      = ptr + StrLen("&time=");
        dateTime = StrAToI(ptr);
        dateTime = (dateTime > NNSH_WRITE_DELAYSECOND) ? (dateTime - NNSH_WRITE_DELAYSECOND) : 1;
        StrIToA(timeBuff, dateTime);
        for (loop = 0; loop < sizeof(timeBuff); loop++)
        {
            if (timeBuff[loop] == '\0')
            {
                break;
            }
            *(ptr + loop) = timeBuff[loop];
        }
    ...
    if ((NNshGlobal->NNsiParam)->writeSequence2 != 0)
    {
        ...
        ret = NNshHttp_comm(HTTP_SENDTYPE_POST_SEQUENCE2, url, (NNshGlobal->NNsiParam)->cookieStoreBuffer, param, 
                            HTTP_RANGE_OMIT, HTTP_RANGE_OMIT, NULL);
    }
    else
    {
        ret = NNshHttp_comm(HTTP_SENDTYPE_POST, url, (NNshGlobal->NNsiParam)->cookieStoreBuffer, param, 
                            HTTP_RANGE_OMIT, HTTP_RANGE_OMIT, NULL);
    }
    if (ret != errNone)
    {
...
{ 
    ...
    setBBSNameAndTime(bbsData.bbsType, bbsData.boardNick,
                      buffer, (NNshGlobal->NNsiParam)->writeBufferSize);
    StrCat(buffer, "&kibi=dango&submit=%90V%8BK%83X%83%8C%83b%83h%8D%EC%90%AC%89%E6%96%CA%82%D6");
