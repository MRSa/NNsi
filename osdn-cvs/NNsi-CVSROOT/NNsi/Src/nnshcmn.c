/*============================================================================*
 *
 *  $Id: nnshcmn.c,v 1.38 2008/10/26 15:02:41 mrsa Exp $
 *
 *  FILE: 
 *     nnshcmn.c
 *
...
Err SendToLocalMemopad(Char *title, Char *data)
{
    ...
    if (CheckInstalledResource_NNsh('appl', 'PMem') != false)
    {
        // memoがインストールされているか確認する    
        // (もし、インストールされていた場合には、無理やりそれに送る)
        NNsh_DebugMessage(ALTID_INFO, "Change to PMem", " default : ", creatorId);
        exgSocket.target = 'PMem';
    }
