...
static Err openFile_FileStream(Char *name, UInt16 fileMode, NNshFileRef *ref)
{
    ...
    // ファイルモードが createのときだけ、appInfoブロック生成ロジックが動作する
    // (言い換えると、Createしないと、sony FileDB互換にはならない。)
    ...
    // なんか動かないので、、、コメントにする。。。
#if 0
    //////  ファイルのAppInfo領域にFileDB関連の情報を設定  //////

    // FileStreamのdbRefを取得する
    ...
    // DBの LocalIDを取得し、appInfo領域が既に存在するかどうかチェックする
    ...
    if (appInfoId != 0)
    {
        // アプリケーションインフォブロック(FileDB Header領域)が存在した
        // (一旦領域を開放する)
