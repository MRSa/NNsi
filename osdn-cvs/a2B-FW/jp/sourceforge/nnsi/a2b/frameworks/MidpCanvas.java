 ...
 *
 */
public class MidpCanvas  extends Canvas implements CommandListener, IDataProcessing, ILogMessageDisplay, IMidpCyclicTimerReport
{
    ...
    private int       myWidth     = 240;    // 画面幅
    private int       myHeight    = 320;    // 画面縦

    private MidpCyclicTimer  timerThread = null;
     ...
     *   画面の定期更新を指示する
     * @param interval
     * @param isReportProessing
     * @param reportingId
     * @return
     */
    public boolean startCyclicRefresh(long interval, boolean isReportProessing, int reportingId)
    {
    	if (timerThread != null)
    	{
    		return (false);
    	}
        processUpdate = isReportProessing;
        processingId  = reportingId;
        timerThread = new MidpCyclicTimer(this, interval);
        timerThread.start();
        return (true);
    }
    
    /**
     *   周期的に更新情報を受信する (MidpCyclicTimerからの応答)
     */
    public boolean reportCyclicTimeout(int count)
    {
    	...
    	try
    	{
            if ((processUpdate == true)&&(activeCanvas != null))
            {
            	activeCanvas.updateData(processingId);
            }
    	}
