...
public class MidpResourceFileUtils
{
	...
	static public String convertInternalFileLocation(String orgName)
	{
        ...
        if ((platForm.indexOf("WX34") >= 0)||(platForm.indexOf("WX01K") >= 0))
        {
        	...
        	if (orgName.indexOf("file:///data/") >= 0)
        	{
        		// データフォルダの場合...
        		newName = "file:///D:/" + orgName.substring(8); //   "file:///" を置換
        	}
        	else
        	{
        	    // SDカードの場合...
        		newName = "file:///F:/" + orgName.substring(12); //  "file:///SD:/" を置換
        	}
        }
        else if ((platForm.indexOf("WS023T") >= 0)||(platForm.indexOf("WS018KE") >= 0)||(platForm.indexOf("WS009KE") >= 0))
        {
            // WS023T or WS018KEの時には、ディレクトリを補正する。。。
            newName = "file:///affm/nand0/PS/data/" + orgName.substring(19); // "file:///DataFolder/" を置換
        }
        else if ((platForm.indexOf("WX320K") >= 0)||(platForm.indexOf("WX33") >= 0)||(platForm.indexOf("WX35") >= 0))
        {
            // WX320K/WX33x/WX35xの時には、ディレクトリ位置を補正する。
			//  (8は、'file:///' の文字列を切り取るため)
