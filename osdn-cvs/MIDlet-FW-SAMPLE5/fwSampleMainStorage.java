...
{
    private fwSampleStorage storage = null;
     ...
     *
     */
    public fwSampleMainStorage(IDataProcessing destination, fwSampleStorage recordStorage, MidpFileMediaUtils utls)
    {
        ...
        setTime = storage.getTimerSetTime();
    }
    ...
    public boolean reportCyclicTimeout(int count)
    {
        if (timerStatus == ISampleMainCanvasStorage.TIMER_STARTED)
        {
            ...
            if (currentTime >= setTime)
            {
                ...
                // サウンドファイル名が指定されていたか？
                String soundFile = storage.getSoundFileName();
                if ((soundFile != null)&&(soundFile.length() > 0))
                {
                    // サウンドを鳴らす！
                    mediaUtils.PlaySound(soundFile, "audio/midi");
                }
    ...
    public boolean setSelectedCommandButtonId(int buttonId)
    {
        ...
        storage.setTimerSetTime(setTime);
        return (true);
    }
    ...
    public int nextSceneToChange(int screenId)
    {
        ...
        if (screenId == fwSampleFactory.SELECTION_SCREEN)
        {
            ...
            {
                return (fwSampleFactory.DIR_SCREEN);
            }
        }
